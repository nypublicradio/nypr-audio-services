...
test('addAction adds action calls to the queue', function(assert) {
  let service = this.subject();

  let called = [];

  service.addAction('finishedTrack', {priority: 1}, function() {
    called.push(1);
  });

  service.addAction('finishedTrack', {priority: 2}, function() {
    called.push(2);
  });

  let finishedTrackQueue = service.get('queues.finishedTrack');

  assert.equal(finishedTrackQueue.length, 2, "finishedTrackQueue should have two items in it");
});
...
test('long running actions work as expected', function(assert) {
  ...
  service.runQueue('finishedTrack').then((results) => {
...
test('runQueue runs the actions in priority order', function(assert) {
  ...
  service.addAction('finishedTrack', {priority: 10}, function() {
    called.push(3);
  });

  service.runQueue('finishedTrack').then(() => {
